...
public class GetFullBookDetailsExercise
{
    ...
    {
        var sql = $@"
    SELECT library.books.book_id as {nameof(Book.BookId)},
    ...
    FROM library.books
    JOIN library.author_wrote_book_items as laspeder on books.book_id = laspeder.book_id
    join library.authors on authors.author_id = laspeder.author_id
